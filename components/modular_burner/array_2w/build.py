"""
Road Roaster 2W Modular Burner Array Standalone Build Script

Builds modular_burner_array_2w.FCStd and exports standard orthogonal and isometric PNG renders.
"""
...
def build():
    init_materials()
    doc_name = "modular_burner_array_2w"
    fcstd_path = os.path.join(script_dir, f"{doc_name}.FCStd")
    doc = FreeCAD.newDocument(doc_name)
    doc.saveAs(fcstd_path)

    grp = create_modular_burner_array_2w_component(doc)
    ensure_assembly_visible(doc)
    doc.recompute()

    report = get_mass_properties(doc)
    print(format_mass_report(report, title="Road Roaster 2W Modular Burner Array Mass Report"))
    ...
    print("Road Roaster 2W modular burner array build complete.")
