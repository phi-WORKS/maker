"""
Modular Ceramic Infrared Burner Cassette Standalone Build Script

Builds modular_ceramic_burner.FCStd and exports standard orthogonal and isometric PNG renders.
"""
...
def build():
    init_materials()
    doc_name = "modular_ceramic_burner"
    fcstd_path = os.path.join(script_dir, f"{doc_name}.FCStd")
    doc = FreeCAD.newDocument(doc_name)
    doc.saveAs(fcstd_path)

    grp = create_modular_ceramic_burner_component(doc)
    ensure_assembly_visible(doc)
    doc.recompute()

    report = get_mass_properties(doc)
    print(format_mass_report(report, title="Modular Ceramic Burner Cassette Mass Report"))
    ...
    print("Modular ceramic burner build complete.")
