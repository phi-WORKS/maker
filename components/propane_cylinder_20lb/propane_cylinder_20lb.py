...
def create_propane_cylinder_20lb_component(doc, placement=None):
    ...
    # Color Palette
    TANK_WHITE = (0.90, 0.91, 0.93, 0.0)     # Durable White/Light Gray Powder Coat
    VALVE_BRASS = (0.85, 0.65, 0.20, 0.0)    # Brass OPD Service Valve Body
    KNOB_BLACK = (0.15, 0.15, 0.15, 0.0)     # Black Polymer Handwheel
    REG_ZINC = (0.65, 0.68, 0.72, 0.0)       # Zinc Low-Pressure Regulator
    ...
    # 2. Lower Dished Dome (approximated by truncated cone + cylinder)
    lower_dome = Part.makeCone(FOOT_R, CYL_R, DOME_H, FreeCAD.Vector(0, 0, FOOT_H), FreeCAD.Vector(0, 0, 1))

    # 3. Main Cylindrical Body Shell
    main_cyl = Part.makeCylinder(CYL_R, SHELL_H, FreeCAD.Vector(0, 0, Z_shell_bot), FreeCAD.Vector(0, 0, 1))

    # 4. Upper Dished Dome
    upper_dome = Part.makeCone(CYL_R, COLLAR_R, DOME_H, FreeCAD.Vector(0, 0, Z_shell_top), FreeCAD.Vector(0, 0, 1))
    ...
    set_obj_visuals(doc, obj_tank, TANK_WHITE)
    ...
    set_obj_visuals(doc, obj_valve, VALVE_BRASS)
    ...
    set_obj_visuals(doc, obj_knob, KNOB_BLACK)
    ...
    set_obj_visuals(doc, obj_reg, REG_ZINC)
