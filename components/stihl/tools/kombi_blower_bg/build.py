"""
STIHL BG-KM Blower Attachment Standalone Build Script

Builds kombi_blower_bg.FCStd, prints mass properties, and exports 7 standard orthogonal and isometric PNG renders.
"""
...
from kombi_blower_bg import create_kombi_blower_bg_component
...
def build():
    doc_name = "kombi_blower_bg"
    fcstd_path = os.path.join(script_dir, f"{doc_name}.FCStd")
    doc = FreeCAD.newDocument(doc_name)
    doc.saveAs(fcstd_path)

    grp = create_kombi_blower_bg_component(doc)
    doc.recompute()

    report = get_mass_properties(grp)
    print(format_mass_report(report, title="STIHL BG-KM Axial Blower Mass Report"))
    ...
    print("STIHL BG-KM Blower build complete.")
