"""
STIHL FH-KM Power Scythe Attachment Standalone Build Script

Builds kombi_scythe_fh.FCStd, prints mass properties, and exports 7 standard orthogonal and isometric PNG renders.
"""
...
from kombi_scythe_fh import create_kombi_scythe_fh_component
...
def build():
    doc_name = "kombi_scythe_fh"
    fcstd_path = os.path.join(script_dir, f"{doc_name}.FCStd")
    doc = FreeCAD.newDocument(doc_name)
    doc.saveAs(fcstd_path)

    grp = create_kombi_scythe_fh_component(doc)
    doc.recompute()

    report = get_mass_properties(grp)
    print(format_mass_report(report, title="STIHL FH-KM Power Scythe Mass Report"))
    ...
    print("STIHL FH-KM Power Scythe build complete.")
