...
- 💧 [**`WET_VS_DRY_STRATEGY.md`**](WET_VS_DRY_STRATEGY.md): Thermodynamic analysis of moisture permeation, latent steam condensation, and soil conductivity protocols (dry gliding vs. damp heat-soak).
...
| :---: | :---: | :--- | :---: |
| **v0.6.0** | ![v0.6.0 Snapshot](road-roaster_iso.png) | **Decomposed Torch Controls, Continuous Chassis Side Rails & Coaxial Axle Stacking**: Transformed hood into a directional heat scoop with rearward offset apex ($Y_{apex} = +110\text{ mm}$), steep rear heat deflector, and long forward-sloping radiant roof ramp. Added continuous $1.5'' \times 3/16''$ chassis side rails with integrated axle pivot ears, clean coaxial axle stacking (`[Wheel] ── [Side Rail] ── [Handle]`), top-crossbar squeeze cockpit (`components/torch_control_handle/`), dual cross-rails with side-mounted 1 lb propane harness, chassis burner nozzle (`components/torch_burner_head/`), and continuous solid $1/2''$ steel through-axle. | 🟡 **`[IN PROGRESS]`** |
