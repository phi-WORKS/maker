...
def build():
    ...
    model.Label = "Road Roaster - Towable Thermal Weed Shock Sled"
    ...
    # Save outputs
    fc_file = os.path.join(script_dir, "sled.FCStd")
    png_file = os.path.join(script_dir, "sled.png")
    ...
    if HAS_GUI:
        try:
            ...
            if gui_d:
                base_prefix = os.path.join(script_dir, "sled")
                export_orthogonal_views(gui_d, base_prefix)
        except Exception as e:
            print(f"Render error: {e}")

    FreeCAD.closeDocument("road_roaster")
    print(f"Successfully created Road Roaster master model & multi-view renders in {script_dir}")
