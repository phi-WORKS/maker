...
def build_v3():
    ...
    v3_doc = FreeCAD.newDocument("caddy_v3")
    v3_doc.Label = "Flame Sled v3 - Handle Leaning Forward Towards Puller"

    # Parametric VarSet (dims)
    dims = v3_doc.addObject("App::VarSet", "dims")
    ...
    TORCH_ANGLE = 35.0   # 35 deg forward/rearward incline from vertical
    ...
    hood_obj = v3_doc.addObject("Part::Feature", "Pyramidal_Hood_Heat_Flange")
    ...
    gussets_obj = v3_doc.addObject("Part::Feature", "Corner_Gussets_AngleIron")
    ...
    skids_obj = v3_doc.addObject("Part::Feature", "Dual_Skid_Runners")
    skids_obj.Shape = dual_skids

    # 4. Overhead Torch Mounting Frame (Angled 35 deg forward to hold wand leaning towards puller)
    f_height = 80.0
    f_width = APEX_W + 50.0
    f_thick = 4.76
    ...
    # Sleeve angled 35 deg forward towards -Y (puller direction)
    ...
    torch_frame_obj = v3_doc.addObject("Part::Feature", "Overhead_Torch_Mounting_Frame")
    torch_frame_obj.Shape = leg_l.fuse(leg_r).fuse(bridge).fuse(sleeve_ring)

    # 5. Harbor Freight Torch 91037 (v3: Handle Leaning 180° Forward towards puller)
    # Burner flame nozzle points down & rearward (+Y)
    ...
    # Wand shaft extends from nozzle up & FORWARD (wand_lean_dir towards -Y front puller)
    wand_start = nozzle_pos + wand_lean_dir * 80.0
    wand_length = 500.0
    wand_shaft = Part.makeCylinder(9.525, wand_length, wand_start, wand_lean_dir)

    # Handle body at end of forward-leaning wand
    ...
    tank_pos = FreeCAD.Vector(-300.0, -200.0, 400.0) # Operator carrying tank at front-left (-Y)
    ...
    burner_obj = v3_doc.addObject("Part::Feature", "HF_Burner_Head_Nozzle")
    burner_obj.Shape = burner_head

    wand_obj = v3_doc.addObject("Part::Feature", "Torch_Chrome_Wand_Shaft")
    wand_obj.Shape = wand_shaft

    handle_obj = v3_doc.addObject("Part::Feature", "HF_Blue_Torch_Handle")
    handle_obj.Shape = handle_body.fuse(grip_insert).fuse(squeeze_lever)

    brass_valve_obj = v3_doc.addObject("Part::Feature", "Brass_Flow_Control_Knob")
    brass_valve_obj.Shape = brass_knob

    igniter_obj = v3_doc.addObject("Part::Feature", "Piezo_Igniter_Module")
    igniter_obj.Shape = igniter_housing.fuse(igniter_button).fuse(igniter_wire)

    tank_obj = v3_doc.addObject("Part::Feature", "Propane_Cylinder_1lb")
    ...
    hitch_obj = v3_doc.addObject("Part::Feature", "Front_Clevis_Hitch_With_StopTab")
    hitch_obj.Shape = left_ear.fuse(right_ear).fuse(stop_tab)

    pin_obj = v3_doc.addObject("Part::Feature", "Clevis_Pin_38in")
    ...
    towbar_obj = v3_doc.addObject("Part::Feature", "Forward_Rigid_Tow_Bar_5ft")
    ...
    for obj, color in color_map.items():
        set_vis(v3_doc, obj, color)

    if HAS_GUI:
        gui_d = FreeCADGui.getDocument(v3_doc.Name)
        if gui_d:
            gui_d.activeView().viewIsometric()
            gui_d.activeView().fitAll()
        FreeCADGui.updateGui()

    fc_v3 = os.path.join(script_dir, "sled_v03.FCStd")
    png_v3 = os.path.join(script_dir, "sled_v03.png")
    v3_doc.saveAs(fc_v3)

    if HAS_GUI:
        gui_d = FreeCADGui.getDocument(v3_doc.Name)
        hide_list = [towbar_obj, tank_obj]
        render_camera_view(gui_d, png_v3, "Isometric", hide_list)

    FreeCAD.closeDocument("caddy_v3")
    print(f"Built Version 3: {fc_v3} and {png_v3}")
