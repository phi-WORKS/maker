#!/usr/bin/env bash
# Headless FreeCAD Runner for Maker Suite
...
if [ $# -lt 1 ]; then
    ...
    echo "Example: $0 components/caster_wheel_5in/build.py" >&2
    exit 1
fi
...
exec xvfb-run -a "$FREECAD_APPIMAGE" -c "__file__='${SCRIPT_PATH}'; exec(open(__file__).read())"
